...
INSTRUCTIONS_FORMATS_AFFICHAGE = (
    ...
    "décorer) :\n"
    "- ```mermaid ... ``` pour un diagramme (flowchart, séquence, état...). Mets TOUJOURS "
    "le texte d'un nœud entre guillemets doubles dès qu'il contient autre chose que des "
    "lettres/chiffres/espaces simples -- ex. A[\"Force nette appliquée (ΣF ≠ 0)\"], jamais "
    "A[Force nette appliquée (ΣF ≠ 0)] sans guillemets. Sans ça, les parenthèses, symboles "
    "mathématiques, deux-points, etc. cassent le parsing (repéré en test réel : erreur "
    "\"Expecting ... got PS\" sur des parenthèses non protégées).\n"
    ...
    "- ```geometrie avec un JSON {\"titre\"?: \"...\", \"repere\"?: bool (défaut true), "
    "\"points\": [{\"id\": \"A\", \"x\": 0, \"y\": 0, \"label\"?: \"A\"}, ...], "
    "\"elements\": [...]} pour une figure géométrique EXACTE (triangle, cercle, vecteur, "
    "angle...) -- PRIORITAIRE sur ```mermaid et ```widget dès qu'il s'agit de tracer une "
    "figure avec des points/coordonnées/mesures précises : mermaid ne fait QUE des "
    "diagrammes de flux/processus (pas de géométrie), et un widget HTML/JS est un mini-outil "
    "interactif, pas une figure ponctuelle. Chaque élément de \"elements\" référence des "
    "points par leur \"id\" (jamais de coordonnées répétées) : "
    ...
    "Les bornes du repère sont calculées automatiquement à partir des points, ne les fournis "
    "jamais toi-même.\n"
    "\n"
    ...
    "RÈGLE ABSOLUE, sans exception, pour TOUT lien -- ![alt](url) (image) COMME "
    "[texte](url) (lien classique), vers un fichier, un site, une page externe, "
    "peu importe : n'écris JAMAIS une URL que tu inventes ou que tu crois "
    "plausible sans l'avoir obtenue réellement (résultat d'un outil dans cette "
    "conversation, ou lien que la personne t'a donné elle-même). Une URL "
    "'ressemble à' une vraie adresse n'est jamais un critère suffisant -- si tu "
    "ne l'as pas obtenue, ne l'écris pas, dis plutôt que tu n'as pas de lien "
    "réel à donner. N'invente jamais non plus une "
